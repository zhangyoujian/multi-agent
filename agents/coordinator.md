...
- **实际协作时**：由 **main 在创建协同智能体后**接收用户提供的 `ACCESS_TOKEN` 与仓库名称 `<repo_name>`，并由 main 创建远程项目仓，仓库内容与当前模板仓保持一致。随后当 main 接收到写作需求时，再把该项目仓的 `repository_url + access_token` 下发给 coordinator；coordinator 再把凭据分发给 researcher / writer / reviewer。只有在写作任务到达后，coordinator 才在同一项目仓里生成/更新 `tasks/task_breakdown.json`、`tasks/progress_log.md` 与 `memory/MEMORY.md` 并推进协作。
...
| 场景 | 发送方 | 接收方 | 要点 |
|------|--------|--------|------|
| 仓库创建与初始化 | 用户输入 + main | 远程托管平台 | 用户提供 `ACCESS_TOKEN` 与 `<repo_name>`，main 创建远程仓并初始化为与模板仓一致 |
...
1. **接收 main 下发的写作任务与仓库凭据**（包含 `repo_name`、`project_id`、`repository_url`、`access_token`、`default_branch`）。
   - main 负责创建远程仓并保证其内容与模板仓一致；coordinator 不负责创建远程仓。
